...
(function(){
  ...
  const VERSION_SOURCES=[
    ...
    {label:'Versão publicada no servidor',makeUrl:()=>new URL('./version.json',window.location.href).href,remote:false},
    {label:'Versão publicada /public',makeUrl:()=>new URL('./public/version.json',window.location.href).href,remote:false}
  ];
  ...
  async function fetchPublishedVersion(){
    ...
    for(const source of VERSION_SOURCES){
      try{
        ...
        const result={version,source:source.label,remote:Boolean(source.remote)};
        if(source.remote)return result;
